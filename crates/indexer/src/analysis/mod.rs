...
use crate::analysis::types::{
    ConsolidatedRelationship, DefinitionNode, DirectoryNode, EndpointNode, FileNode, FqnType, GraphData,
...
impl AnalysisService {
    ...
    pub fn analyze_results(
        ...
        let mut endpoint_nodes: Vec<EndpointNode> = Vec::new();
        ...
        for (language, results) in results_by_language {
            ...
            // Collect endpoints from Java analyzer
            if language == SupportedLanguage::Java {
                let mut java_endpoints = self.java_analyzer.take_endpoints();
                endpoint_nodes.append(&mut java_endpoints);
                log::info!(
                    "Collected {} Java endpoints from analyzer",
                    endpoint_nodes.len()
                );
            }
        }

        // Collect service calls from Java analyzer
        let mut service_call_nodes = Vec::new();
        let mut java_service_calls = self.java_analyzer.take_service_calls();
        service_call_nodes.append(&mut java_service_calls);
        log::info!(
            "Collected {} Java service calls from analyzer",
            service_call_nodes.len()
        );

        let analysis_time = start_time.elapsed();
        log::info!(
            "Analysis completed in {:?}: {} directories, {} files, {} definitions ({} total locations), {} imported symbols ({} total locations), {} endpoints, {} service calls, {} total relationships",
            analysis_time,
            directory_nodes.len(),
            file_nodes.len(),
