...
impl<'a> FileProcessor<'a> {
    ...
    pub fn process(&self) -> ProcessingResult {
        ...
        // Use unified pipeline for all languages (ParserType + rules + analyzer)
        {
            ...
            // 3. Run rules to find matches (only for ast-grep results)
            ...
            let matches = match &parse_result {
                UnifiedParseResult::AstGrep(ast_result) => {
                    ...
                    run_rules(&ast_result.ast, Some(&self.path), &rule_manager)
                }
                UnifiedParseResult::Ruby(_) => {
                    // Ruby uses prism parser, no ast-grep rules
                    Vec::new()
                }
                UnifiedParseResult::TypeScript(_) => {
                    // TypeScript uses swc parser, no ast-grep rules
                    Vec::new()
                }
            };
        ...
        let result = match language {
            ...
            SupportedLanguage::Python => {
                if let UnifiedParseResult::AstGrep(ast_result) = parse_result {
                    ...
                    Err(anyhow::anyhow!(
                        "Expected AstGrep parse result for Python file '{}'",
                        self.path
                    ))
                }
            }
            SupportedLanguage::Kotlin => {
                if let UnifiedParseResult::AstGrep(ast_result) = parse_result {
                    ...
                    Err(anyhow::anyhow!(
                        "Expected AstGrep parse result for Kotlin file '{}'",
                        self.path
                    ))
                }
            }
            SupportedLanguage::Java => {
                if let UnifiedParseResult::AstGrep(ast_result) = parse_result {
                    ...
                    Err(anyhow::anyhow!(
                        "Expected AstGrep parse result for Java file '{}'",
                        self.path
                    ))
                }
            }
            SupportedLanguage::CSharp => {
                if let UnifiedParseResult::AstGrep(ast_result) = parse_result {
                    ...
                    Err(anyhow::anyhow!(
                        "Expected AstGrep parse result for CSharp file '{}'",
                        self.path
                    ))
                }
            }
            SupportedLanguage::TypeScript => {
                ...
                match parse_result {
                    UnifiedParseResult::AstGrep(ast_result) => match analyzer.analyze(ast_result) {
                        Ok(analysis_result) => Ok((
                    ...
                    UnifiedParseResult::TypeScript(typescript_result) => {
                    ...
                    _ => Err(anyhow::anyhow!(
                        "Expected TypeScript or AstGrep parse result for TypeScript file '{}'",
                        self.path
                    )),
                }
            }
            SupportedLanguage::Rust => {
                if let UnifiedParseResult::AstGrep(ast_result) = parse_result {
                    ...
                    Err(anyhow::anyhow!(
                        "Expected AstGrep parse result for Rust file '{}'",
                        self.path
                    ))
                }
