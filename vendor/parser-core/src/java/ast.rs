use crate::{
    ...
    java::{
        ...
        types::{
            AstRootNode, JavaAnnotation, JavaAnnotationArgument, JavaDefinitionInfo, JavaDefinitionMetadata, JavaDefinitionType,
            JavaDefinitions, JavaExpression, JavaFqn, JavaFqnPart, JavaFqnPartType, JavaImports,
            JavaMethodParameter, JavaReferenceInfo, JavaReferenceType, JavaReferences, JavaType,
        },
...
fn parse_class_declaration_node(
    ...
    let annotations = parse_annotations_from_node(node);
    ...
    parser_result
        ...
        .push(JavaDefinitionInfo::new_with_metadata(
            ...
            Arc::new(parser_result.current_scope.clone()),
            class_range,
            JavaDefinitionMetadata::Class { super_types, annotations },
        ));
...
fn parse_enum_declaration_node(
    ...
    parser_result
        ...
        .push(JavaDefinitionInfo::new_with_metadata(
            ...
            Arc::new(parser_result.current_scope.clone()),
            enum_range,
            JavaDefinitionMetadata::Class { super_types, annotations: parse_annotations_from_node(node) },
        ));
...
fn parse_method_declaration_node(
    ...
    let parameters = extract_method_parameters(node);
...
fn parse_constructor_declaration_node(
    node: &Node<StrDoc<SupportLang>>,
    parser_result: &mut JavaParseResult,
) -> bool {
    let constructor_name = node.field("name");
    if constructor_name.is_none() {
        return false;
    }

    let constructor_name = constructor_name.unwrap().text().into_owned();
    let constructor_range = node_to_range(node);

    // Extract constructor parameters
    let parameters = extract_method_parameters(node);

    let fqn_part = JavaFqnPart::new(JavaFqnPartType::Constructor, constructor_name.clone(), constructor_range);
    let annotations = parse_annotations_from_node(node);
    ...
    parser_result
        ...
        .push(JavaDefinitionInfo::new_with_metadata(
            ...
            Arc::new(parser_result.current_scope.clone()),
            constructor_range,
            JavaDefinitionMetadata::Constructor {
                parameters,
                annotations,
            },
        ));
...
fn parse_field_declaration_node(
    ...
    if !is_lambda {
        ...
        // Extract field initializer value
        let initializer = variable_declarator
            .field("value")
            .map(|v| v.text().into_owned());

        // Extract modifiers (public, private, static, final, etc.)
        let modifiers = extract_modifiers_from_node(node);
            ...
            parser_result
                ...
                .push(JavaDefinitionInfo::new_with_metadata(
                    ...
                    JavaDefinitionMetadata::Field {
                        ...
                        annotations: Vec::new(),
                        initializer: None,  // Record parameters don't have initializers
                        modifiers: Vec::new(),  // Record parameters implicitly final
                    },
                ));
...
fn parse_annotations_from_node(node: &Node<StrDoc<SupportLang>>) -> Vec<JavaAnnotation> {
    let mut annotations = Vec::new();

    // Look for modifiers child node which contains annotations
    for child in node.children() {
        if child.kind().as_ref() == "modifiers" {
            for modifier_child in child.children() {
                let is_annotation = modifier_child.kind().as_ref() == node_types::ANNOTATION;
                let is_marker = modifier_child.kind().as_ref() == node_types::MARKER_ANNOTATION;

                if is_annotation || is_marker {
                    let annotation_text = modifier_child.text();
                    eprintln!(
                        "[PARSER] Attempting to parse annotation: is_annotation={}, is_marker={}, text='{}'",
                        is_annotation,
                        is_marker,
                        annotation_text
                    );
                    if let Some(annotation) = parse_single_annotation(&modifier_child) {
                        eprintln!(
                            "[PARSER] ✅ Parsed annotation '{}' with {} args",
                            annotation.name,
                        ...
                        eprintln!(
                            "[PARSER] ❌ Failed to parse annotation from: '{}'",
                            annotation_text
                        );
                    }
                }
    ...
    if !annotations.is_empty() {
        eprintln!(
            "[PARSER] Total annotations parsed for this node: {}",
            annotations.len()
        );
    }
...
fn parse_single_annotation(node: &Node<StrDoc<SupportLang>>) -> Option<JavaAnnotation> {
    ...
    if let Some(args) = node.field("arguments") {
        for arg_child in args.children() {
            if arg_child.kind().as_ref() == node_types::ELEMENT_VALUE_PAIR {
                // Named argument: @Annotation(name = "value")
                if let (Some(key), Some(value)) = (arg_child.field("key"), arg_child.field("value")) {
                    arguments.push(JavaAnnotationArgument {
                        name: Some(key.text().to_string()),
                        value: value.text().to_string(),
                    });
                }
            } else if !arg_child.text().trim().is_empty() && arg_child.text() != "(" && arg_child.text() != ")" && arg_child.text() != "," {
                // Single value argument: @Annotation("value")
                arguments.push(JavaAnnotationArgument {
                    name: None,
                    value: arg_child.text().to_string(),
                });
            }
        }
    }
    
    Some(JavaAnnotation { name, arguments })
}

fn extract_method_parameters(node: &Node<StrDoc<SupportLang>>) -> Vec<crate::java::types::JavaMethodParameter> {
    ...
    if let Some(params_node) = node.field("parameters") {
        for param_child in params_node.children() {
            if param_child.kind().as_ref() == node_types::FORMAL_PARAMETER {
                if let Some(param) = extract_single_parameter(&param_child) {
...
fn extract_single_parameter(node: &Node<StrDoc<SupportLang>>) -> Option<crate::java::types::JavaMethodParameter> {
    
    let parameter_type = node.field("type")?;
    let parameter_name = node.field("name")?;
    
    let java_type = parse_type_node(&parameter_type)?;
    let name = parameter_name.text().to_string();
...
fn parse_node(node: &Node<StrDoc<SupportLang>>, parser_result: &mut JavaParseResult) -> bool {
    match node.kind().as_ref() {
        ...
        node_types::CONSTRUCTOR_DECLARATION => {
            parse_constructor_declaration_node(node, parser_result)
        }
...
/// Extract modifiers from a node (public, private, static, final, etc.)
fn extract_modifiers_from_node(node: &Node<StrDoc<SupportLang>>) -> Vec<String> {
    let mut modifiers = Vec::new();

    // Look for modifiers child node
    for child in node.children() {
        if child.kind().as_ref() == "modifiers" {
            // Each modifier is a child of the modifiers node
            for modifier_child in child.children() {
                let modifier_text = modifier_child.text().into_owned();

                // Filter out annotations (we handle those separately)
                // Only keep actual modifiers like public, private, static, final, etc.
                if !modifier_text.starts_with('@') && !modifier_text.is_empty() {
...
mod tests {
    ...
    fn test_super_types_are_included_in_declaration() {
        ...
        match my_class.metadata.as_ref().unwrap() {
            JavaDefinitionMetadata::Class { super_types, annotations: _ } => {
