...
pub fn load_rules_from_yaml(yaml: &str, lang: SupportedLanguage) -> Vec<RuleConfig<SupportLang>> {
    ...
    match from_yaml_string::<SupportLang>(yaml, &globals) {
        Ok(rules) => rules
            ...
            .collect(),
        Err(e) => {
            // Handle case where YAML is empty or only contains metadata without rules
            tracing::debug!(
                "Failed to parse YAML rules for {lang:?}: {e}. This may be expected for languages with no rules configured."
            );
            Vec::new()
        }
    }
...
#[derive(Clone)]
pub struct ParseResult<'a, T = AstGrep<StrDoc<SupportLang>>> {
...
pub type DefaultParseResult<'a> = ParseResult<'a, AstGrep<StrDoc<SupportLang>>>;

/// Core trait for language parsers
pub trait LanguageParser<T = AstGrep<StrDoc<SupportLang>>> {
    fn parse<'a>(&self, code: &'a str, file_path: Option<&'a str>) -> Result<ParseResult<'a, T>>;

    fn language(&self) -> SupportedLanguage;
}

/// Extension trait for parsers that use ast-grep
pub trait AstGrepParser: LanguageParser<AstGrep<StrDoc<SupportLang>>> {
...
impl LanguageParser<AstGrep<StrDoc<SupportLang>>> for GenericParser {
    fn parse<'a>(
        &self,
        code: &'a str,
        file_path: Option<&'a str>,
    ) -> Result<ParseResult<'a, AstGrep<StrDoc<SupportLang>>>> {
...
impl AstGrepParser for GenericParser {}
...
pub enum ParserType {
    AstGrep(GenericParser),
    Ruby(crate::ruby::parser::RubyParser),
    TypeScript(crate::typescript::parser::TypeScriptParser),
}

/// Unified parse result that can hold either AST-grep or Ruby prism results
pub enum UnifiedParseResult<'a> {
    AstGrep(
        ParseResult<
            'a,
            ast_grep_core::AstGrep<
...
impl ParserType {
    pub fn for_language(language: SupportedLanguage) -> Self {
        match language {
            ...
            _ => Self::AstGrep(GenericParser::new(language)),
        }
    }

    pub fn language(&self) -> SupportedLanguage {
        match self {
            Self::AstGrep(parser) => parser.language(),
    ...
    pub fn parse<'a>(
        ...
        match self {
            Self::AstGrep(parser) => {
                ...
                Ok(UnifiedParseResult::AstGrep(result))
            }
            Self::Ruby(parser) => {
                let result = parser.parse(code, file_path)?;
...
impl<'a> UnifiedParseResult<'a> {
    pub fn language(&self) -> SupportedLanguage {
        match self {
            Self::AstGrep(result) => result.language,
    ...
    pub fn file_path(&self) -> Option<&str> {
        match self {
            Self::AstGrep(result) => result.file_path.as_deref(),
...
"#;
        ...
        let result = load_rules_from_yaml(valid_yaml, SupportedLanguage::Ruby);
        assert!(!result.is_empty(), "Valid YAML should return rules");

        // Test with completely invalid YAML - should return empty Vec instead of panicking
        let invalid_yaml = "not: yaml: content: [unclosed";
        let result = load_rules_from_yaml(invalid_yaml, SupportedLanguage::Ruby);
        assert!(result.is_empty(), "Invalid YAML should return empty Vec");

        // Test with incomplete YAML (missing rule field) - should return empty Vec
        let incomplete_yaml = r#"
id: test-rule
language: Ruby
"#;
        let result = load_rules_from_yaml(incomplete_yaml, SupportedLanguage::Ruby);
        assert!(result.is_empty(), "Incomplete YAML should return empty Vec");
    }
    ...
    fn test_parser_type_creation() {
        ...
        assert!(matches!(python_parser, ParserType::AstGrep(_)));
    }
    ...
    fn test_unified_parser_python() -> Result<()> {
        ...
        assert!(matches!(result, UnifiedParseResult::AstGrep(_)));
