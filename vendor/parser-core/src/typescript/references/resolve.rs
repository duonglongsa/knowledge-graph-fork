...
use crate::typescript::references::types::{
    TypeScriptReferenceInfo, TypeScriptReferenceType, TypeScriptSymbol, TypeScriptSymbolType,
...
mod tests {
    ...
    use crate::typescript::references::utils::{
    ...
    fn test_basic_global_function_resolution() {
        ...
        let (node_fqn_map, imports, expressions) = parse_ast(&parse_result.ast);
    ...
    fn test_shared_scope_expressions() {
        ...
        let expressions = fold_expressions::<TypeScriptExpression>(&expressions);
    ...
    fn test_crawl_scope_with_this_method_calls() {
        ...
        for expression in &expressions {
            if expression.string.contains("this.")
                && expression.symbols.iter().any(|s| {
                    s.symbol_type
                        == crate::typescript::references::types::TypeScriptSymbolType::MethodCall
                })
            {
                            ...
                            && let Some(children) = context.get_scope_children(symbol)
                        {
                            ...
                            if !method_names.is_empty() {
                                ...
                                for expr_symbol in &_expr.symbols {
                                    if expr_symbol.symbol_type == crate::typescript::references::types::TypeScriptSymbolType::MethodCall
                                            && method_names.contains(&expr_symbol.name) {
                                                println!("    ✓ Method '{}' resolved to class '{}'", expr_symbol.name, symbol_name);
                                            }
                                }
                            }
                        }
